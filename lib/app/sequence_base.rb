...
module Inferno
  module Sequence
    ...
    class SequenceBase
      ...
      # Filters responses to return an array where, for every klass indicated in the +klasses+ array, 
      # every applicable resource from the server is repesented in an element.
      # If +klasses+ is undefined or empty, it defaults to nil, which will result in returning 
      # every resource available in the server.
      # 
      # Returns <code>[resource, resource, ...]</code> (groomed array of every resource of 
      # any provided klass provided by the server), +nil+ if no replies
      ...
      ##
      # Retrieves all bundles from server when requesting every resource 
      # of each type in the +klasses+ array.
      # If +klasses+ is undefined or empty, it defaults to nil, which will 
      # return bundles of every resource with no regard for type.
      #
      # Returns <code>[reply, reply, ...]</code> (+array+ of every unchanged 
      # server response), +nil+ if no replies
      ...
      ##
      # Returns +nil+ if +param+ is falsy. If +param+ is truthy, returns +param.clone.to_a+ 
      # if possible, otherwise returns +param+ as the only element in a new +Array+
      ...
      # (Defining a param will never increase the scope of the check, in fact in most cases 
      # it will narrow the scope)
      #
