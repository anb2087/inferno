      ...
      test 'Demonstrates that the server can supply must supported elements' do
        metadata {
          ...
          versions :r4
            ...
            skip 'Could not find Goal.lifecycleStatus in the provided resource' unless (@instance.must_support_confirmed.include? "Goal.lifecycleStatus") || can_resolve_path(@goal, 'lifecycleStatus')
            @instance.must_support_confirmed += 'Goal.lifecycleStatus'
        
            skip 'Could not find Goal.description in the provided resource' unless (@instance.must_support_confirmed.include? "Goal.description") || can_resolve_path(@goal, 'description')
            @instance.must_support_confirmed += 'Goal.description'
        
            skip 'Could not find Goal.subject in the provided resource' unless (@instance.must_support_confirmed.include? "Goal.subject") || can_resolve_path(@goal, 'subject')
            @instance.must_support_confirmed += 'Goal.subject'
        
            skip 'Could not find Goal.target in the provided resource' unless (@instance.must_support_confirmed.include? "Goal.target") || can_resolve_path(@goal, 'target')
            @instance.must_support_confirmed += 'Goal.target'
        
            skip 'Could not find Goal.target.due[x] in the provided resource' unless (@instance.must_support_confirmed.include? "Goal.target.due[x]") || can_resolve_path(@goal, 'target.duedate')
            @instance.must_support_confirmed += 'Goal.target.due[x]'
        
            skip 'Could not find Goal.target.due[x] in the provided resource' unless (@instance.must_support_confirmed.include? "Goal.target.due[x]") || can_resolve_path(@goal, 'target.dueDuration')
            @instance.must_support_confirmed += 'Goal.target.due[x]'
