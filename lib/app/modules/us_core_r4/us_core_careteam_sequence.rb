      ...
      test 'Demonstrates that the server can supply must supported elements' do
        metadata {
          ...
          versions :r4
            ...
            skip 'Could not find CareTeam.status in the provided resource' unless (@instance.must_support_confirmed.include? "CareTeam.status") || can_resolve_path(@careteam, 'status')
            @instance.must_support_confirmed += 'CareTeam.status'
        
            skip 'Could not find CareTeam.subject in the provided resource' unless (@instance.must_support_confirmed.include? "CareTeam.subject") || can_resolve_path(@careteam, 'subject')
            @instance.must_support_confirmed += 'CareTeam.subject'
        
            skip 'Could not find CareTeam.participant in the provided resource' unless (@instance.must_support_confirmed.include? "CareTeam.participant") || can_resolve_path(@careteam, 'participant')
            @instance.must_support_confirmed += 'CareTeam.participant'
        
            skip 'Could not find CareTeam.participant.role in the provided resource' unless (@instance.must_support_confirmed.include? "CareTeam.participant.role") || can_resolve_path(@careteam, 'participant.role')
            @instance.must_support_confirmed += 'CareTeam.participant.role'
        
            skip 'Could not find CareTeam.participant.member in the provided resource' unless (@instance.must_support_confirmed.include? "CareTeam.participant.member") || can_resolve_path(@careteam, 'participant.member')
